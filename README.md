...
- When developing locally, the single command `bin/EWTest-httpd.mv` runs your web server and takes care of serving static files
- When deploying changes (after a git push), `EWTest-update restart` deploys changes and restarts services on your server
- Rolling back the server to an earlier version is a simple as `EWTest-update restart v0.1.2`
